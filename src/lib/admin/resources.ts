...
export const RESOURCES: Record<ResourceKey, ResourceConfig> = {
  brands: {
    ...
    fields: [
      ...
      { kind: "text", name: "website", label: "Brand website", maxLength: 300, group: "Presentation" },
      {
        ...
        name: "segment",
        label: "Segment",
        hint: "Which group the /brands page lists this brand under.",
        options: [
          { value: "SOFTWARE_CLOUD", label: "Software & cloud" },
          { value: "BUSINESS_HARDWARE", label: "Business hardware" },
      ...
      {
        kind: "text",
        name: "partnerBadgeUrl",
        label: "Partner badge file",
        maxLength: 200,
        hint: "A file in public/badges/, shown beside the designation once it is confirmed and public.",
        group: "Partner status",
      },
  ...
  services: {
    ...
    fields: [
      ...
      { kind: "lines", name: "technologies", label: "Technologies", hint: "One per line.", group: "Content" },
      {
        ...
        required: true,
        hint: 'A JSON array, one object per step: [{"step": 1, "title": "…", "description": "…"}, …]',
        group: "Content",
      },
      { kind: "number", name: "displayOrder", label: "Display order", min: 0, max: 10_000, group: PUBLICATION_GROUP },
  ...
  posts: {
    ...
    fields: [
      ...
      { kind: "relation", name: "authorId", label: "Author", resource: "user", hint: "Leave empty for no byline.", group: "Identity" },
  ...
  industries: {
    ...
    fields: [
      ...
      },
      {
        ...
        maxItems: 20,
        hint: "One brand URL slug per line, exactly as it appears at /brands/{slug} — e.g. microsoft, adobe. Rendered as real links on the sector's page. An unrecognised slug is silently skipped, never a broken link.",
        group: "Content",
      },
      {
        kind: "lines",
        ...
        maxItems: 20,
        hint: "One service URL slug per line, from /services/{slug}.",
        group: "Content",
      },
      {
        kind: "lines",
        ...
        maxItems: 20,
        hint: "One category URL slug per line.",
        group: "Content",
      },
      { kind: "checkbox", name: "published", label: "Published", defaultChecked: true, group: PUBLICATION_GROUP },
      { kind: "number", name: "displayOrder", label: "Display order", min: 0, max: 10_000, group: PUBLICATION_GROUP },
  ...
  jobs: {
    ...
    fields: [
       ...
       */
      { kind: "number", name: "salaryMinMinor", label: "Salary from (minor units)", min: 0, hint: "Paise for INR, fils for AED. Leave both blank not to advertise pay — most roles here do not.", group: "Pay" },
      { kind: "number", name: "salaryMaxMinor", label: "Salary to (minor units)", min: 0, group: "Pay" },
      { kind: "select", name: "salaryCurrency", label: "Currency", options: [
        { value: "INR", label: "₹ INR" },
        { value: "AED", label: "AED" },
      ], group: "Pay" },
