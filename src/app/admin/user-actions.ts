 ...
 * arrived first.
 */
async function issueInvite(user: { id: string; name: string; email: string }): Promise<{
  link: string;
  delivered: boolean;
}> {
 ...
 * and an administrator with no way to fix it. The link is a credential for the
 * next three days, so the wording says so rather than presenting it as a
 * convenience.
 */
function undeliveredMessage(email: string, link: string): string {
  return [
    ...
    link,
    `Check Settings → Outbound email if this keeps happening.`,
  ].join("\n\n");
}
...
export async function createUser(
  ...
  if (!invite.delivered) {
    ...
    return { status: "error", message: undeliveredMessage(user.email, invite.link) };
  }

  return {
    status: "success",
    message: `${user.name} has been added as ${role === "ADMIN" ? "an administrator" : role === "SALES" ? "a sales user" : "a customer"}. They have been emailed a link to choose a password.`,
  };
}
