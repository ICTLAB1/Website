...
| `payment-settings.mjs` | The payment credential form: enabling without keys is refused, a malformed key id is refused, and — the reason this suite exists — neither secret ever reaches the browser or the database in plain text. Also that a blank field leaves a stored secret alone, since the form can never show one. |
