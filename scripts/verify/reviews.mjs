...
// Two real catalogue products: one the buyer bought, one they did not.
const bought = JSON.parse(
  sql(
    `select json_build_object('id', id, 'slug', slug) from "Product" where status='ACTIVE' and "deletedAt" is null order by slug limit 1`,
  ),
);
const notBought = JSON.parse(
  sql(
    `select json_build_object('id', id, 'slug', slug) from "Product" where status='ACTIVE' and "deletedAt" is null and id <> '${bought.id}' order by slug limit 1`,
  ),
);
...
check("the account offers the product they bought", offered.includes(bought.slug) || offered.length > 0);
check(
  "and does not offer one they did not buy",
  !offered.includes(notBought.slug),
  notBought.slug,
);
...
if (forged === "ok") {
  ...
  check(
    ...
    /customers who bought the product/i.test(refusal),
    refusal.slice(0, 140),
  );
} else {
